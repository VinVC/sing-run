...
- **订阅URL**: 用于 `sing-run update-nodes`（可省略，省略时自动查找 `data/<短名>` 本地文件）
- 颜色自动分配，无需配置
...
sing-run jp --check-nodes             # 真实代理检测所有源的日本节点
sing-run jp --check-nodes --source edn # 检测指定源
sing-run jp --node next               # 切换下一个节点
sing-run jp --node prev               # 切换上一个节点
sing-run jp --node 5                  # 切换到指定节点
```

`--check-nodes` 会为每个节点临时启动普通代理模式的 sing-box，并通过该节点访问测试 URL；默认检测所有源、并发数为 4，也可用 `--source` 限定单个源。它不会修改当前节点、源或正在运行的实例。
检测结果会缓存到本机 `~/.sing-run/node-checks/`，供 `sing-run <区域> --nodes` 显示上次状态；缓存不保存节点服务器、端口、UUID、密码或订阅 URL。
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  ProviderA (pa) · 3 个节点
    ...
    #     名称                                    类型      上次检测
    ──  ────────────────────────────────────────  ────    ────────
     0  日本1                                     vmess     OK 0.24s 06-12 16:39
     1  日本2                                     vmess     -
     2  日本3                                     vmess     -
...
- `[当前]` 标记当前使用的源
- `→` 标记当前使用的节点
- `上次检测` 来自最近一次 `--check-nodes` 缓存，`-` 表示未检测或节点已变化
- 不显示服务器域名

### `sing-run jp --check-nodes`

真实代理检测指定区域的节点，默认覆盖所有源：
...
超时: 8s
  ...
  #     状态    耗时        节点                                    结果
  ──    ────    ────────    ────────────────────────────────────────  ────
  0     OK      0.442604s   日本1                                   HTTP 204
  1     FAIL    -           日本2                                   curl 超时

结果: 1 个可用，1 个失败
切换节点: sing-run jp --node <编号>
```

常用选项：

```bash
sing-run jp --check-nodes --limit 3
sing-run jp --check-nodes --timeout 5
sing-run jp --check-nodes --concurrency 8
sing-run jp --check-nodes --url https://www.gstatic.com/generate_204
```
...
域名规则对所有实例生效，生成时会同时使用 `domain` 和 `domain_suffix` 匹配根域名及其所有子域名。
...
├── sing-run-node-check.sh             # 节点真实代理检测、检测缓存
...
| 节点检测 | `sing-run-node-check.sh` | 临时代理探测节点可用性，维护本机检测缓存 |
